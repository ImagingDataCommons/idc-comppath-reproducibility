...
class BatchIterator:
    ...
    ----------
    _wsi: 'WSI'
        Attribute storing the WSI object currently considered.
    _tile_size: int
        Size of the tiles. 
    _required_pixel_spacing: float
        Pixel spacing at which the tiles should be extracted.
    ...
    """
    def __init__(self, wsi: WSI, tile_size: int, required_pixel_spacing: float, 
        ...
        ----------
        wsi: 'WSI'
            WSI object to be considered for now.
        tile_size: int
            Size of the tiles. 
        ...
        self._tile_positions = self._compile_all_tile_positions(\
            wsi, tile_size, required_pixel_spacing, coverage)
        self._accept_function = accept_function

    @staticmethod
    def _compile_all_tile_positions(wsi: WSI, tile_size: int, required_pixel_spacing: float, coverage: float) -> list:
        """
    ...
    def __next__(self) -> Tuple[np.ndarray, List[Tuple[int, int]]]:
        ...
        while self._tile_index < len(self._tile_positions) and curr_batch_size < self._batch_size: 
            ...
            tile = self._wsi.get_tile(tile_pos, self._tile_size, self._required_pixel_spacing)
